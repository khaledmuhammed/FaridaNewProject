            ...
            checkCode() {
                if (this.code.length && !this.isLoading) {
                    ...
                    axios.post('/coupon', {
                        ...
                        // in case of coupon type is "shipping"
                        if(coupon.couponable_type === "shipping"){
                            /*
                             first make sure that the shippingPrice in the store is equal to the shippingPrice in 
                             the cart, so if the buyer hit the "validate button" many time, it not apply the discount 
                             more than once
                            */
                            this.$store.commit('updateShippingPrice', this.$root.$refs.shipping.shippingPrice)
                            
                            if(parseFloat(coupon.minimum) <= this.$store.getters.getTotalPrice){
                                this.$store.commit('updateCoupon', coupon)
                                if(coupon.calc === 'fixed'){
                                    var new_shipping_price = this.$store.state.cart.prices.shipping - coupon.amount
                                    if(new_shipping_price < 0) new_shipping_price = 0
                                }else{ /*coupon.calc is in percentage*/
                                    var new_shipping_price = this.$store.state.cart.prices.shipping - (this.$store.state.cart.prices.shipping * coupon.amount / 100)
                                }
                                this.$store.commit('updateShippingPrice', new_shipping_price)
                            }else{ // if the coupon minimum is less than the total price
