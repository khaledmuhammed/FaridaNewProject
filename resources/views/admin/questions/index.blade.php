        ...
        <div class="col-xs-12">
            <div class="box">
                <div class="box-header">
                    <a class="btn btn-success btn-new-element" href="{{action('Admin\QuestionController@create')}}">Add
                        Question</a>
                </div>
                <div class="box-body">
                    <div class="table-responsive">
                        <table class='table table-hover'>
                            ...
                            @foreach($questions as $question)
                                <tr>
                                    <td>{{$question->id}}</td>
                                    <td>{{$question->user->firstName}}
                                        {{$question->user->lastName}}</td>
                                    <td>{{$question->product->name}}</td>
                                    <td>{{$question->text}}</td>
                                    <td>
                                        <a class="btn btn-success btn-xs"
                                           href="{{action('Admin\QuestionController@show',$question->id)}}">@lang('Show')</a>
                                        <a class="btn btn-primary btn-xs"
                                           href="{{action('Admin\QuestionController@edit',$question->id)}}">@lang('Edit')</a>
                                        <button class="remove-model delete-model btn  btn-danger  btn-xs"
                                                data-path="questions" data-obj="{{$question->id}}"
