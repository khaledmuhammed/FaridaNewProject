...
class PaymentFailed extends Notification
{
    ...
    {
        return (new MailMessage)->markdown('mail.statuses.payment_failed', ['order' => $this->order])
                    ->subject('طلبك رقم #'.$this->order->id.' : '.$this->orderHistory->status->name_ar);
    }

    public function toSMS($notifiable)
    {
        return (new Message())->message("مرحباً ".$this->order->username."\nنعتذر منك، هناك خلل في محاولتك لإتمام عملية الدفع ، يرجى الدخول لصفحة الطلبات ومحاولة الدفع مرة أخرى. لن تحتاج لإعادة الطلب من جديد .")
