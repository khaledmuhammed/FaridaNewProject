...
class Shipping extends Notification
{
    ...
    {
        return (new MailMessage)->markdown('mail.statuses.shipping', ['order' => $this->order])
                    ->subject('طلبك رقم #'.$this->order->id.' : '.$this->orderHistory->status->name_ar);
    }

    public function toSMS($notifiable)
    {
        return (new Message())->message("مرحباً ".$this->order->username."\nتم شحن طلبك رقم: ".$this->order->id." ، وسيتواصل معك المندوب لتوصيل طلبك إليك . \n".$this->orderHistory->notes)
